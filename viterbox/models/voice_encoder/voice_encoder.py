...
def get_frame_step(overlap: float, rate: float, hp: VoiceEncConfig):
    assert 0 <= overlap < 1
    if rate is None:
...
class VoiceEncoder(nn.Module):
    def __init__(self, hp=VoiceEncConfig(), onnx_path=None):
        super().__init__()
    ...
    def embeds_from_mels(self, mels: Union[Tensor, List[np.ndarray]], mel_lens=None, as_spk=False, batch_size=32, **kwargs):
    ...
    def embeds_from_wavs(self, wavs: List[np.ndarray], sample_rate, as_spk=False, batch_size=32, trim_top_db: Optional[float]=20, **kwargs):
