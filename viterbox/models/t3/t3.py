...
def _ensure_BOT_EOT(text_tokens: Tensor, hp):
    ...
    assert (
        (text_tokens == hp.start_text_token).int().sum() >= batch_size
    ), "missing start_text_token"

    assert (
        (text_tokens == hp.stop_text_token).int().sum() >= batch_size
...
class T3(nn.Module):
    def __init__(self, hp=T3Config()):
        ...
        self.text_head = nn.Linear(
            ...
            hp.text_tokens_dict_size,
            bias=False,
        )

        self.speech_head = nn.Linear(
            self.cfg.hidden_size,
            hp.speech_tokens_dict_size,
            bias=False,
        )
    ...
    def forward(
        ...
        *,
        ...
        for i in range(batch_size):
            ...
            text_latents[i, : text_token_lens[i]] = hidden_states[
                i,
                len_cond:text_end,
            ]

            speech_latents[i, : speech_token_lens[i]] = hidden_states[
                i,
    ...
    def loss(
        ...
        *,
        ...
        mask_text = (
            torch.arange(len_text, device=device)[None]
            >= text_token_lens[:, None].to(device)
        )

        mask_speech = (
            torch.arange(len_speech, device=device)[None]
            >= speech_token_lens[:, None].to(device)
        )

        masked_text = text_tokens.to(device).masked_fill(mask_text, ignore_id)
        masked_speech = speech_tokens.to(device).masked_fill(mask_speech, ignore_id)
    ...
    def inference(
        ...
        *,
        ...
        for i in tqdm(
            ...
            if stop_on_eos and next_token.view(-1).item() == self.hp.stop_speech_token:
                break
